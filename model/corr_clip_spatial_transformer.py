...
class ClipMatcher(nn.Module):
    ...
    def forward(self, clip, query, fix_backbone=True):
        ...
        # downsample the per-frame features
        # transformer for leveraging temporal information
        ...
        for cur_head in self.down_heads:
            ...
            if list(feat.shape[-2:]) == [self.resolution_transformer, self.resolution_transformer]:
                # feat = rearrange(feat, '(b t) c h w -> b c (h w t)', b=b)
                # for layer in self.transformer_layer:
                #     feat = layer(query=feat, key=feat, query_embed=self.pe_3d, key_embed=self.pe_3d)
                # feat = rearrange(feat, 'b c (h w t) -> (b t) c h w', b=b, h=self.resolution_transformer, w=self.resolution_transformer)
                feat = rearrange(feat, '(b t) c h w -> b (h w t) c', b=b)
                for layer in self.transformer_layer:
                    feat = layer(feat + self.pe_3d)
                feat = rearrange(feat, 'b (h w t) c -> (b t) c h w', b=b, h=self.resolution_transformer, w=self.resolution_transformer)
