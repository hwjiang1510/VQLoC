...
from utils.model_utils import PositionalEncoding1D, positionalencoding1d, positionalencoding2d
...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.pe_2d = positionalencoding2d(128, self.clip_feat_size_coarse, self.clip_feat_size_coarse)
        self.pe_2d = 0.02 * rearrange(self.pe_2d, '(h w) c -> c h w', h=self.clip_feat_size_coarse)
    ...
    def forward(self, clip, query, fix_backbone=True):
        ...
        feat = torch.cat([clip_feat, query_feat, self.pe_2d.repeat(b*t,1,1,1).to(clip_feat.device)], dim=1)
        feat = self.fuse(feat)
