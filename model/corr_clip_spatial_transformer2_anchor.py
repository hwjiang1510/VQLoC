...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.out = nn.Sequential(
            ...
            nn.BatchNorm1d(128),
            nn.LeakyReLU(inplace=True),
            nn.Linear(128, n_base_sizes*n_aspect_ratios*5),
        )
    ...
    def forward(self, clip, query, fix_backbone=True):
        ...
        for head in self.down_heads:
            ...
            if list(clip_feat.shape[-2:]) == [self.resolution_transformer]*2:
                ...
                clip_feat = rearrange(clip_feat, 'b (t h w) c -> (b t) (h w) c', b=b, t=t, h=self.resolution_transformer, w=self.resolution_transformer)
                break
        
        clip_feat = rearrange(clip_feat, 'B n c -> (B n) c')
        pred = self.out(clip_feat)   # [b*t*h*w,c] -> [b*t*h*w,n*m*5]
        pred = rearrange(pred, '(B n) c -> B n c', B=b*t)
        pred = rearrange(pred, 'B (h w) (n m c) -> B (h w n m) c', h=self.resolution_transformer, w=self.resolution_transformer, n=n_base_sizes, m=n_aspect_ratios, c=5)
        pred[:,:,:4] += self.anchors.to(pred.device)
        pred = rearrange(pred, '(b t) N c -> b t N c', b=b, t=t)
