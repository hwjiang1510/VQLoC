...
def build_backbone(config):
    ...
    if name == 'dino':
        ...
        assert type in ['vitl14-336']
        backbone, _ = clip.load('ViT-L/14@336px')
...
class ClipMatcher(nn.Module):
    ...
    def extract_feature(self, x, return_h_w=False):
        if self.backbone_name == 'dino':
            ...
            out = self.backbone.visual.get_visual_feature(x)
            #__import__('pdb').set_trace()
            h, w = int(h_origin // self.down_rate), int(w_origin // self.down_rate)
            dim = out.shape[-1]
            out = out.reshape(b, h, w, dim).permute(0,3,1,2)  # [b,c,h,w]
            out = F.interpolate(out, size=(32,32), mode='bicubic')
            return out
